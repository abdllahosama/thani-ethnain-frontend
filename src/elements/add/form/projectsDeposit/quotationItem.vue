<template>
  ...
  <div class="row pt-4 quotation-item mb-3 pb-3">
    <div class="col-xl-3">
      ...
      <div class="search-container" v-click-outside="removeItemList">
        ...
        <ul class="list-unstyled search-selects" v-if="searchProducts.length > 0 && showItemsList">
          <li v-for="product in searchProducts" :key="product.id" @click="changeProduct(product)" :class="{'focusd': product.focused}" :title="product.description">
          {{product.name}} 
          <span
    ...
    <div class="col-xl-2">
      ...
      <input type="number" class="form-control w-100"  :class="{ 'is-invalid' : item.quantityError}" placeholder="0"  v-model="item.quantity" @keyup="updateItem()">
    </div>
    <div class="col-xl-2">
