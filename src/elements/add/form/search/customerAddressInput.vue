  ...
  <div class="form-group col-md-6 row">
    ...
    <div class="col-md-7">
      <div class="search-container" v-click-outside="removeCustomerList">
        <input type="search"
               autocomplete="off"
               v-model="$parent.item.customer.address1"
               class="form-control"
        ...
        <ul class="list-unstyled search-selects" v-if="searchCustomers.length > 0 && $parent.item.customer_id == 0 && showCustomersList">
          <li v-for="customer in searchCustomers"
              ...
              :class="{'focusd': customer.focused}">{{customer.address1}}</li>
        </ul>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      searchCustmer() {
        var search = this.$parent.item.customer.address1
        if (this.addedCustomer) {
          ...
          var customers = this.$database.customers.sort(this.$dynamicSort({ orderBy: "address1", orderType: "desc" })).filter(el => editsearch.test(el.address1) && el.address1 != null).slice(0, 20)
