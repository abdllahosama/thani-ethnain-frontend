  ...
  <div class="form-group col-md-6 row">
    ...
    <div class="col-md-7">
      <div class="search-container" v-click-outside="removePartnerList">
        <input type="search"
               autocomplete="off"
               v-model="$parent.item.partner.mobile"
               class="form-control"
               maxlength="255"
        ...
        <ul class="list-unstyled search-selects" v-if="searchPartners.length > 0 && $parent.item.partner_id == 0 && showPartnersList">
          <li v-for="partner in searchPartners"
              ...
              :class="{'focusd': partner.focused}">{{partner.mobile}}</li>
        </ul>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      searchCustmer() {
        var search = this.$parent.item.partner.mobile
        if (this.addedPartner) {
          ...
          var partners = this.$database.partners.sort(this.$dynamicSort({ orderBy: "mobile", orderType: "desc" })).filter(el => editsearch.test(el.mobile) && el.mobile != null).slice(0, 20)
