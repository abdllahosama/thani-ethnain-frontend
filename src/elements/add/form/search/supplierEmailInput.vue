  ...
  <div class="form-group col-md-6 row">
    ...
    <div class="col-md-7">
      <div class="search-container" v-click-outside="removeSupplierList">
        <input type="search"
               autocomplete="off"
               v-model="$parent.item.supplier.email"
               class="form-control"
               maxlength="255"
        ...
        <ul class="list-unstyled search-selects" v-if="searchSuppliers.length > 0 && $parent.item.supplier_id == 0 && showSuppliersList">
          <li v-for="supplier in searchSuppliers"
              ...
              :class="{'focusd': supplier.focused}">{{supplier.email}}</li>
        </ul>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      searchCustmer() {
        var search = this.$parent.item.supplier.email
        if (this.addedSupplier) {
          this.addedSupplier = false
        } else {
          this.showSuppliersList = true
          var editsearch = new RegExp(search, 'i')
          var suppliers = this.$database.suppliers.sort(this.$dynamicSort({ orderBy: "email", orderType: "desc" })).filter(el => editsearch.test(el.email) && el.email != null).slice(0, 20)
      ...
      selectSupplier(e) {
        if (
          ...
          if (eselected != -1) {
            this.addSupplierFromSearch(esupplier.id, esupplier.email)
