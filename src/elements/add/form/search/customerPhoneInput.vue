  ...
  <div class="form-group col-md-6 row">
    ...
    <div class="col-md-7">
      <div class="search-container" v-click-outside="removeCustomerList">
        <input type="search"
               autocomplete="off"
               v-model="$parent.item.customer.mobile"
               class="form-control"
               maxlength="255"
        ...
        <ul class="list-unstyled search-selects" v-if="searchCustomers.length > 0 && $parent.item.customer_id == 0 && showCustomersList">
          <li v-for="customer in searchCustomers"
              ...
              :class="{'focusd': customer.focused}">{{customer.mobile}}</li>
        </ul>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      searchCustmer() {
        var search = this.$parent.item.customer.mobile
        if (this.addedCustomer) {
          ...
          var customers = this.$database.customers.sort(this.$dynamicSort({ orderBy: "mobile", orderType: "desc" })).filter(el => editsearch.test(el.mobile) && el.mobile != null).slice(0, 20)
