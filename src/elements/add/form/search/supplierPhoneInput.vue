  ...
  <div class="form-group col-md-6 row">
    ...
    <div class="col-md-7">
      <div class="search-container" v-click-outside="removeSupplierList">
        <input type="search"
               autocomplete="off"
               v-model="$parent.item.supplier.mobile"
               class="form-control"
               maxlength="255"
        ...
        <ul class="list-unstyled search-selects" v-if="searchSuppliers.length > 0 && $parent.item.supplier_id == 0 && showSuppliersList">
          <li v-for="supplier in searchSuppliers"
              ...
              :class="{'focusd': supplier.focused}">{{supplier.mobile}}</li>
        </ul>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      searchCustmer() {
        var search = this.$parent.item.supplier.mobile
        if (this.addedSupplier) {
          this.addedSupplier = false
        } else {
          this.showSuppliersList = true
          var editsearch = new RegExp(search, 'i')
          var suppliers = this.$database.suppliers.sort(this.$dynamicSort({ orderBy: "mobile", orderType: "desc" })).filter(el => editsearch.test(el.mobile) && el.mobile != null).slice(0, 20)
