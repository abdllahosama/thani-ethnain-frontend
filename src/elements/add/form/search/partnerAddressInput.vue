  ...
  <div class="form-group col-md-6 row">
    ...
    <div class="col-md-7">
      <div class="search-container" v-click-outside="removePartnerList">
        <input type="search"
               autocomplete="off"
               v-model="$parent.item.partner.address1"
               class="form-control"
        ...
        <ul class="list-unstyled search-selects" v-if="searchPartners.length > 0 && $parent.item.partner_id == 0 && showPartnersList">
          <li v-for="partner in searchPartners"
              ...
              :class="{'focusd': partner.focused}">{{partner.address1}}</li>
        </ul>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      searchCustmer() {
        var search = this.$parent.item.partner.address1
        if (this.addedPartner) {
          ...
          var partners = this.$database.partners.sort(this.$dynamicSort({ orderBy: "address1", orderType: "desc" })).filter(el => editsearch.test(el.address1) && el.address1 != null).slice(0, 20)
