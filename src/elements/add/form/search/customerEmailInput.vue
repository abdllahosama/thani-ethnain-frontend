  ...
  <div class="form-group col-md-6 row">
    ...
    <div class="col-md-7">
      <div class="search-container" v-click-outside="removeCustomerList">
        <input type="search"
               autocomplete="off"
               v-model="$parent.item.customer.email"
               maxlength="255"
               class="form-control"
        ...
        <ul class="list-unstyled search-selects" v-if="searchCustomers.length > 0 && $parent.item.customer_id == 0 && showCustomersList">
          <li v-for="customer in searchCustomers"
              ...
              :class="{'focusd': customer.focused}">{{customer.email}}</li>
        </ul>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      searchCustmer() {
        var search = this.$parent.item.customer.email
        if (this.addedCustomer) {
          ...
          var customers = this.$database.customers.sort(this.$dynamicSort({ orderBy: "email", orderType: "desc" })).filter(el => editsearch.test(el.email) && el.email != null).slice(0, 20)
      ...
      selectCustomer(e) {
        if (
          ...
          if (eselected != -1) {
            this.addCustomerFromSearch(ecustomer.id, ecustomer.email)
