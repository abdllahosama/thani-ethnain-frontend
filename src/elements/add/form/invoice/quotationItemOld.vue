      ...
      <div class="row">
        <div class="col-xl-9">
          ...
          <!--div class="mt-1" v-if="item.product_id == ''">
            ...
            <label for="addToProducts" class="mr-1 prevent-select">اضافة الي المنتجات</label>
          </div-->
          <textarea
            class="form-control w-100"
            :placeholder="
...
export default {
  ...
  methods: {
    ...
    changeProduct(Product) {
      this.mainPrice = Product.price
      this.pricesValues = Product.priceArray
      this.item.product_id = Product.id;
      this.item.maxQuantity = Product.track_quantity == 1 ? Product.quantity : "";
      this.item.product_name = Product.name;
      this.item.description = Product.description;
      this.item.unit_id = Product.unit_id;
      this.item.price = this.$parent.$parent.item.customer_id ? this.pricesValues[this.$parent.$parent.item.customer_id] : this.mainPrice;
      this.item.quantity = 1;
      this.item.count = 1;
      if (Product.tax_id != 0) {
  ...
  watch: {
    "$parent.$parent.item.customer_id": function () {
      this.item.price = this.$parent.$parent.item.customer_id ? this.pricesValues[this.$parent.$parent.item.customer_id] : this.mainPrice;
      this.updateItem();
    },
