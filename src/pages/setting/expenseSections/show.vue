                        ...
                        <table class="table table-sm table-nowrap card-table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <tableTh :text="$t('expenses.expenseDate')" :sort="'date'"></tableTh>
                                    <tableTh :text="$t('expenses.thatsAbout')" :sort="'description'"></tableTh>
                                    <tableTh :text="$t('expenses.note')" :sort="'notes'"></tableTh>
                                    <tableTh :text="$t('expenses.amount')" :sort="'cost'"></tableTh>
                                    <tableTh :text="$t('expenses.settings')"></tableTh>
                                </tr>
                            </thead>
                            <loader v-if="loading == 1"></loader>
                            <tbody class="list">
                                <emptyTable v-if="items.length == 0 && loading == 0" :text="$t('expenses.thereAreNoExpenses')"></emptyTable>
                                <tr v-for="(item, index) in items" :key="item.id">
                                    ...
                                    <td class="text-right edit-td">
                                        <router-link :to="'/expenses/'+items[index].id" class="btn btn-light">
                                            <i class="fas fa-search"></i>
                                        </router-link>
                                        <router-link :to="'/expenses/'+items[index].id+'/edit'" class="btn btn-light" v-if="$user.admin || $user.role.expenses_edit">
                                            <i class="fas fa-edit"></i>
                                        </router-link>
                                        <a @click="sureDelete(index)" class="btn btn-light" v-if="$user.admin || $user.role.expenses_delete">
...
<script>
    ...
    export default {
        ...
        methods: {
            getitem() {
                axios.get(this.$linkGnirator(this.path + '/' + this.$route.params.id), {
                    ...
                    .then(response => {
                        ...
                        this.items = this.item.expenses.data;
                        this.totalPages = this.item.expenses.last_page;
                        this.page = this.item.expenses.current_page;
                        this.loaded = true;
                        this.loading = 0;
                    });
            },
            ...
            deleteItem() {
                axios.post(this.$linkGnirator('/expenses/' + this.items[this.delete_id].id), {
                        _method: 'DELETE'
                    })
                    .then(response => {
                        this.getitem();
